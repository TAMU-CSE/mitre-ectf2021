...
/// and instantiate an appropriate [encryption handler](crate::crypto::Handler) to handle
...
/// the controller so that they may be used by your encryption handler.
pub trait Handler<C: CryptoHandler + Sized>: Sized + Copy + Clone {
    ...
    /// containing the associated [encryption handler](crate::crypto::Handler). If it is not
